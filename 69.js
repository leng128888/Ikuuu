...
let index = 1;
while (true) {
  const email = $persistentStore.read(`69yunzh${index}`);
  const password = $persistentStore.read(`69yunmm${index}`);
...
async function executeCheckin() {
  for (const account of accounts) {
    try {
      console.log(`🚀 开始执行69云签到脚本 | 账号: ${account.email}`);
      
      // 1. 登录
      console.log("🔐 正在登录账号...");
      const loginResult = await performLogin(account);
      console.log("✅ 登录成功");
      ...
      // 3. 处理结果
      handleResult(account, checkinResult);
...
async function performLogin(account) {
  ...
  return new Promise((resolve, reject) => {
    $httpClient.post({
      ...
      if (error) return reject(new Error(`登录请求失败: ${error}`));
      if (response.status !== 200) return reject(new Error(`登录失败 | 状态码: ${response.status}`));
      
      try {
        const result = JSON.parse(data);
        if (result.ret !== 1) return reject(new Error(`登录失败 | ${result.msg || '未知错误'}`));
        resolve({ cookie: response.headers['Set-Cookie'] || '', data: result });
      } catch (e) {
...
async function performCheckin(cookie) {
  return new Promise((resolve, reject) => {
    $httpClient.post({
      ...
      if (response.status !== 200) return reject(new Error(`签到失败 | 状态码: ${response.status}`));
...
// 处理结果
function handleResult(account, result) {
  const maskedEmail = maskEmail(account.email);
  const date = new Date();
